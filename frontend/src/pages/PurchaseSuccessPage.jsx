...
const PurchaseSuccessPage = () => {
    ...
    const [error, setError] = useState(sessionId ? null : "No session ID found in the URL");
    useEffect(() => {
		const handleCheckoutSuccess = async (sessionId) => {
			try {
				...
				console.log(error);
				setError(error.response?.data?.message || "Unable to verify checkout session");
			} finally {
        ...
        <div className="max-w-md w-full bg-gray-800 rounded-lg shadow-xl overflow-hidden relative z-10">
            <div className="p-6 sm:p-8">
                <div className="flex justify-center">
                    <CheckCircle className="text-emerald-400 w-16 h-16 md-4"/>
                </div>
                ...
                <p className="text-emerald-400 text-center text-sm mb-6">Check your email for order details and updates.</p>
                 ...
                 <div className="bg-gray-700 rounded-lg p-4 mb-6">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-sm text-gray-400">Oder Number</span>
                        <span className="text-sm font-semibold text-emerald-400">#12345</span>
                    </div>

                    <div className="flex items-center justify-between ">
                        <span className="text-sm text-gray-400">Estimated delivery</span>
                        <span className="text-sm font-semibold text-emerald-400">3-5 business day</span>
                    </div>
                 </div>
